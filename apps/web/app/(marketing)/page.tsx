    ...
    <div className="min-h-screen bg-[#0f1f0f] text-white overflow-x-hidden">
      ...
      <section className="py-20 px-8 bg-[#f8faf6] text-[#1a2f1a]">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            { icon: '✅', title: 'Verified Sellers', desc: 'All our sellers are carefully vetted to ensure quality and reliability.' },
            { icon: '🌱', title: 'Premium Quality', desc: 'Handpicked selection of the healthiest and most beautiful trees.' },
            { icon: '🚚', title: 'Safe Delivery', desc: 'Carefully packaged and delivered to your doorstep with tracking.' },
            { icon: '💳', title: 'Secure Payment', desc: 'Stripe-powered payments ensure your transactions are safe.' }
          ].map((feature, i) => (
            <div key={i} className="bg-white p-8 rounded-2xl text-center border-2 border-[#e8f0e3] hover:-translate-y-2 hover:shadow-xl transition-all duration-300">
              <div className="text-5xl mb-4">{feature.icon}</div>
              <h3 className="text-xl font-semibold mb-3 text-[#2d5016]">{feature.title}</h3>
